...
Fliplet.InteractiveMap.component('add-markers', {
  ...
  data: function data() {
    ...
    }, _babel_runtime_helpers_defineProperty__WEBPACK_IMPORTED_MODULE_2___default()(_ref, "dataSourceId", Fliplet.Widget.getData().markersDataSourceId), _babel_runtime_helpers_defineProperty__WEBPACK_IMPORTED_MODULE_2___default()(_ref, "entries", undefined), _babel_runtime_helpers_defineProperty__WEBPACK_IMPORTED_MODULE_2___default()(_ref, "columns", undefined), _babel_runtime_helpers_defineProperty__WEBPACK_IMPORTED_MODULE_2___default()(_ref, "styleNames", []), _ref;
  },
  computed: {
  ...
  methods: {
    ...
    setActiveMarker: function setActiveMarker(index, forced) {
    ...
    updateMarker: function updateMarker(marker, index) {
    ...
    cancelNameUpdate: function cancelNameUpdate(index) {
    ...
    deleteMarker: function deleteMarker(index) {
      var markerId = this.mappedMarkerData[index].id;
    ...
    nameWithId: function nameWithId(_ref2) {
      var name = _ref2.name,
          id = _ref2.id;
      ...
      if (this.showEditMarkerOverlay) {
        ...
        Fliplet.DataSources.connect(this.dataSourceId).then(function (connection) {
          ...
          connection.find().then(function (records) {
            ...
            _this7.styleNames = [];
            _this7.entries = records;
            _this7.columns = _.keys(records[0].data);
            _this7.markersData = records;
            _this7.mappedMarkerData = _this7.mapMarkerData();

            _this7.dataSourceConnection.commit(_this7.entries, _this7.columns);
    ...
    addMarkers: function addMarkers(fromLoad, options) {
      ...
      if (options.existingMarker) {
        ...
        var markersLength = this.flPanZoomInstances[options.mapId].markers.getAll().length;
        ...
        this.flPanZoomInstances[options.mapId].markers.set(createdMarkers); // Deselect the current marker

        $('.marker').removeClass('active'); // Select new marker
    ...
    onTapHandler: function onTapHandler(e) {
      ...
      if ($(e.target).hasClass('marker')) {
        ...
        var _markerId = $(e.target).data('id');
    ...
    addNewMarker: function addNewMarker(options) {
      ...
      var markerInfo = {
        ...
        y: options && _.hasIn(options, 'existingMarker') ? options.y : position.y // Adds the marker to the map
    ...
    prepareNewMarkerToSave: function prepareNewMarkerToSave(markerInfo, options, position) {
      ...
      this.mappedMarkerData.push(newObj); // Select marker info row

      var newMarkerIndex = _.findIndex(this.mappedMarkerData, function (o) {
        return o.data.name == markerInfo.name;
      });

      this.setActiveMarker(newMarkerIndex, true); // Save
...
/***/ 3:
/*!*******************************************!*\
  !*** multi ./js/interface/add-markers.js ***!
  \*******************************************/
/*! no static exports found */
/***/ (function(module, exports, __webpack_require__) {
